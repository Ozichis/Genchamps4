...
from PyQt5.QtWidgets import QApplication, QLabel, QDialog, QStackedWidget, QInputDialog, QLineEdit, QMessageBox, \
    QCommandLinkButton, QProgressBar, QWidget, QScrollArea, QTextEdit
...
class RegisterChild(QDialog):
    def __init__(self, new_name, country, state, church, zone):
        ...
        self.back_edit.clicked.connect(self.go_back)

    def go_back(self):
        window2.setCurrentIndex(6)

    def change_all(self, name, country, state, church, zone):
        self.new_name = name
        self.country = country
        self.state = state
        self.church = church
                        ...
                        if self.phone_no == '':
                            ...
                            try:
                                ...
                                message = f"""\
                                                                ...
                                                                Welcome to the LoveWorld Children Curriculum App.
                                                                Your child has been registered into our app by {self.new_name}.
                                                                Password: {self.passcode}(Use this Password to login your child into our app).
                                                                Let's keep on spreading the word of God everywhere."""
                                ...
                                conn.commit()
                                cursor.close()
                                QMessageBox.about(self, 'Congratulations',
                                                  'You have registered a child into your class')
                            except:
                                QMessageBox.about(self, 'Sorry', 'It looks like you do not have an internet connection, please retry when you have internet')
...
class AssociateTeacher(QDialog):
    def __init__(self, new_name, country, state, church, zone):
        ...
        self.class_kids.clicked.connect(lambda: self.get_class_kids(self.new_name))

    def get_class_kids(self, name):
        print(name)
        winx.change_name(name)
        window2.setCurrentIndex(8)
    ...
    def show_curriculum(self):
        window2.setCurrentIndex(5)
        print(window2.currentIndex())

    def register(self):
        winker.change_all(self.new_name, self.country, self.state, self.church, self.zone)
        window2.setCurrentIndex(10)

class ClassKids(QDialog):
    def __init__(self, teacher_name):
        super(ClassKids, self).__init__()
        loadUi("children.ui", self)
        self.backer.clicked.connect(self.go_back)
    ...
    def change_name(self, new):
        print(str(new)+'srh')
        objects = self.get_teacher_children(new)
        text = 'Your Children:\n'
        for i in objects.values():
            text += f""""
    Child Name: {i['name']}
...
class ChildPage(QDialog):
                ...
                if messed:
                    ...
                    if your_code != self.secret_number:
                        ...
                        if data['acc_type'] == 'CE Member':
                            ...
                            self.welcome_a_non_ce_member(data['name'])
            except:
    ...
    def check(self):
        ...
        if username == "":
            ...
            for row in cursor:
                if row[0] == username:
                    conns = True
                    data = {'name':row[0], 'password':row[1], 'phone':row[2], 'email':row[3], 'teacher':row[4], 'çlass':row[5], 'birth_date':row[6], 'acc_type':row[7], 'country':row[8], 'state':row[9], 'church':row[10], 'zone':row[11], 'parent_name':row[12], 'coins':row[13]}
                    break
                else:
                    pass
            if conns:
    ...
    def welcome_a_non_ce_member(self, name):
        weeje.change_name(name)
        window2.setCurrentIndex(7)

    def welcomeuser(self, name, password, phone, email, teacher, classe, birth_date, acc_type, country, state, church, zone, parent_name, coins):
        winster.change_all(name, password, phone, email, teacher, classe, birth_date, acc_type, country, state, church, zone)
        window2.setCurrentIndex(11)

class NonCeMember(QDialog):
    def __init__(self, child_name):
        super(NonCeMember, self).__init__()
        self.child_name = child_name
        loadUi("non-ce-member.ui", self)
        self.label.setText(self.label.text().replace('[Child Name]', self.child_name))
        self.label.setStyleSheet("font: 18pt 'MS Shell Dlg 2'; color: white; background-image: url(welcome.jpg);")
        self.label_2.setStyleSheet("background-image: url(curriculum.jpg); border-radius: 50px;")
        self.label_3.setStyleSheet("border-radius: 50px; background-image: url(games.jpg);")
        self.label_4.setStyleSheet("border-radius: 50px; background-image: url(music.jpg);")
        self.games.clicked.connect(self.loadgames)

    def loadgames(self):
        winpi.change_name(self.child_name)
        winpi.change_iv(True)
        window2.setCurrentIndex(9)



    def change_name(self, new):
        self.label.setText(f"Welcome {new}")
        self.child_name = new

class WelcomeUser(QDialog):
    def __init__(self, name, password, phone, email, teacher, classe, birth_date, acc_type, country, state, church, zone, parent_name, coins):
    ...
    def change_all(self, name, password, phone, email, teacher, classe, birth_date, acc_type, country, state, church, zone):
        self.name = name
        self.password = password
        self.phone = phone
        self.email = email
...
class WordScramblers(QDialog):
    ...
    def go_back(self):
        window2.setCurrentIndex(window2.currentIndex()-2)
        ...
        if self.fullnames == '':
            ...
            if self.emails == '':
                ...
                if len(self.passwords) < 6:
                    ...
                    if self.confirms_pass != self.passwords:
                        print(self.confirms_pass)
                        ...
                        if len(self.phones) != 10:
                            ...
                            try:
                                ...
                                message = f"""\
                                                                ...
                                                                Welcome to the LoveWorld Children Curriculum App.
                                                                Your child has been registered into our app."""
                                ...
                                conn = sqlite3.connect("children.db")
                                cursor = conn.execute(
                                    f"INSERT INTO children (UserName, Password, PhoneNumber, Email, Teacher, Class, BirthDate, AccountType, Country, State, Church, Zone, ParentName, Coins) VALUES ('{self.fullnames}', '{self.passwords}', '{self.phones}', '{self.emails}', '', '', '{f'{self.dateEdit.date().year()}/{self.dateEdit.date().month()}/{self.dateEdit.date().day()}'}', 'Outreach Account', '{self.countrsy}', '{self.anti_deep_states}', '', '', '{self.parent_names}', 0)")
                                conn.commit()
                                cursor.close()
                                self.openchild2(self.fullnames)
                            except:
                                QMessageBox.about(self, 'Error', 'There is an error in your email. It is either:\n   ->You do not have an internet connection\n   -> Or Your email does not exist')


    def openchild2(self, name):
        weeje.change_name(name)
        window2.setCurrentIndex(7)
...
winker = RegisterChild("", "", "", "", "")
winx = ClassKids("")
winster = WelcomeUser("", "", "", "", "", "", "", "", "", "", "", "", "", "")
winpi = GamesPage("", False)
...
window2.addWidget(weeje)
window2.addWidget(winx)
window2.addWidget(winpi)
window2.addWidget(winker)
